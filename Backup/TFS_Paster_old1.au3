...
#AutoIt3Wrapper_Res_Fileversion=1.6.0.7
...
Global Const $LOG_PATH = $TEMP_PATH & "\Log"
Global Const $INI_FILE = @ScriptDir & "\Config.ini"
Global Const $OP_LATENCY_MS = 10
...
Func DoJob()
	...
	If Not StringInStr($title, '- Google Chrome') And Not StringInStr($title, '— Mozilla Firefox') Then
		ToolTip("TFS_Paster: Active window is not a browser")
		Sleep(1000)
		Return
	EndIf
...
Func UploadToWeb($filePath)
	...
	While 1
		...
		If Not WebBrowserActive() Then Return SetError(-4, 0, 0)
	WEnd
