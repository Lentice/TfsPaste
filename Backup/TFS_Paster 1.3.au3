#cs ----------------------------------------------------------------------------
 ...
 Script Function:
	Paster text and pictures from word/outlook to TFS web.
...
Func DoJob()
	...
	_Log("Start Job")
	_Clipboard_GetAll($aClip) ; keep origianl contents of clipboard
...
;~ 	HtmlShrink()
	...
	Send("^a")
	Send("^v")
	SaveBinToFile("patched.html", _ClipBoard_GetData($CLIP_HTML_FORMAT))

	Sleep(1000)
	_Clipboard_PutAll($aClip); restore to original clipborad contents
...
Func HtmlShrink()
	...
	$patchedHtmlData = StringRegExpReplace($patchedHtmlData, '(<!--\R[\w\W]*?\/\* Font Definitions \*\/[\w\W]*?-->\s*\r?\n?)', "")
	$patchedHtmlData = StringRegExpReplace($patchedHtmlData, '(<!--\[if [\w\W]*?<!\[endif\]-->\s*\r?\n?)', "")
	$patchedHtmlData = StringRegExpReplace($patchedHtmlData, '(<link[\W\w]*?href=[\w\W]*?>\s*\r?\n?)', "")
	$patchedHtmlData = StringRegExpReplace($patchedHtmlData, '(<v:shapetype[\W\w]*?<\/v:shapetype>\s*\r?\n?)', "")

	$patchedHtmlData = StringRegExpReplace($patchedHtmlData, '<o:p[\w\W]*?>([\W\w]*?)<\/o:p>', "$1")

	$patchedHtmlData = StringRegExpReplace($patchedHtmlData, '([\n\s])lang=(.*?)(\s|>|\n|\r\n)', '$1lang="$2"$3')
	$patchedHtmlData = StringRegExpReplace($patchedHtmlData, '([\n\s])class=(.*?)(\s|>|\n|\r\n)', '$1class="$2"$3')

	$patchedHtmlData = StringRegExpReplace($patchedHtmlData, '<!\[if !supportLists\]>', "")
	$patchedHtmlData = StringRegExpReplace($patchedHtmlData, '<!\[endif\]>', "")

;~ 	$patchedHtmlData = StringRegExpReplace($patchedHtmlData, '(<head[\w\W]*?>[\w\W]*?<\/head>\s*\r?\n?)', "")
;~ 	$patchedHtmlData = StringRegExpReplace($patchedHtmlData, '(<html[\w\W]*?>)', "<html>")
;~ 	$patchedHtmlData = StringRegExpReplace($patchedHtmlData, '(<body[\w\W]*?>)', "<body>")

	$patchedHtmlData = StringRegExpReplace($patchedHtmlData, '^[\W\w]*?(<html[\w\W]*?<\/html>)[\W\w]*', "$1")
EndFunc
...
Func GetRemoteImageUrl()
	...
	For $i = 0 To $localImageCount - 1
		If Not _ImageToClip($localImageFiles[$i]) Then SetError(1, 0, 0)
...
Func ParseRemoteImageUrlFromClip()
	...
	If Not IsArray($tempUrlList) Then Return SetError(2, 0, 0)
...
Func PatchVShape()
	...
	While 1
		...
		$iOffset = @extended
		If @error Or Not IsArray($aArray) Then ExitLoop

		_log("##VShape " & $vShapeCount & ": " & $aArray[0])

		ReDim $vShapeList[$vShapeCount + 1]
		$vShapeList[$vShapeCount] = $aArray[0]
		$vShapeCount += 1
	WEnd
	_log("Got " & $vShapeCount & " vshape from html")

	If $vShapeCount = 0 Then Return 0
...
Func VShapeGetImageSize($vShape)
	...
	For $i = 0 To UBound($keyword) - 1
		Local $value = 0
		Do
			; Check unit pt. pixels = points * 96 / 72
			$aArray = StringRegExp($vShape, '(?i)' & $keyword[$i] & ':\s*(\d+(\.\d+)?)\s*pt', 3)
			...
			; Check unit px
			$aArray = StringRegExp($vShape, '(?i)' & $keyword[$i] & ':\s*(\d+(\.\d+)?)\s*px', 3)
			If Not @error And Number($aArray[0]) <> 0 Then
				$value = Number($aArray[0])
				ExitLoop
			EndIf

			; Check unit cm. 1 cm = 37.795276 px; 1 px = 0.026458 cm
			$aArray = StringRegExp($vShape, '(?i)' & $keyword[$i] & ':\s*(\d+(\.\d+)?)\s*cm', 3)
			If Not @error And Number($aArray[0]) <> 0 Then
				$value = Number($aArray[0]) * 37.795276
				ExitLoop
			EndIf

			; Check unit in. 1 px = 0.010417 in; 1 in = 96 px
			$aArray = StringRegExp($vShape, '(?i)' & $keyword[$i] & ':\s*(\d+(\.\d+)?)\s*in', 3)
			If Not @error And Number($aArray[0]) <> 0 Then
				$value = Number($aArray[0]) * 96
				ExitLoop
			EndIf

			; Check unit mm. 1 mm = 3.779528 px; 1 px = 0.264583 mm
			$aArray = StringRegExp($vShape, '(?i)' & $keyword[$i] & ':\s*(\d+(\.\d+)?)\s*mm', 3)
			If Not @error And Number($aArray[0]) <> 0 Then
