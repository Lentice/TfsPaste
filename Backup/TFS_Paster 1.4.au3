...
#AutoIt3Wrapper_Res_Fileversion=1.4.0.18
...
Func DoJob()
	...
	$patchedHtmlData = HtmlShrinkPre($orignalHtml)
	...
	$patchedHtmlData = HtmlShrinkPost($patchedHtmlData)

	$patchedHtmlData = FixHeadDescription($patchedHtmlData)
...
Func HtmlShrinkPre($htmlData)

	$htmlData = StringRegExpReplace($htmlData, '(<!--\R[\w\W]*?\/\* Font Definitions \*\/[\w\W]*?-->\s*\r?\n?)', "")
	$htmlData = StringRegExpReplace($htmlData, '(<!--\[if [\w\W]*?<!\[endif\]-->\s*\r?\n?)', "")
	$htmlData = StringRegExpReplace($htmlData, '(<link[\W\w]*?href=[\w\W]*?>\s*\r?\n?)', "")

	Local $pattern = '([^"' & "'" & '].*?)(\s|>|\n|\r\n)'
	$htmlData = StringRegExpReplace($htmlData, '([\n\s])lang=' & $pattern, '$1lang="$2"$3')
	$htmlData = StringRegExpReplace($htmlData, '([\n\s])class=' & $pattern, '$1class="$2"$3')

	$htmlData = StringRegExpReplace($htmlData, '<!\[if !supportLists\]>', "")
	$htmlData = StringRegExpReplace($htmlData, '<!\[endif\]>', "")

	Return $htmlData
EndFunc

Func HtmlShrinkPost($htmlData)
	$htmlData = StringRegExpReplace($htmlData, '(<v:shapetype[\W\w]*?<\/v:shapetype>\s*\r?\n?)', "")
	$htmlData = StringRegExpReplace($htmlData, '<o:p[\w\W]*?>([\W\w]*?)<\/o:p>', "$1")
	Return $htmlData
EndFunc

Func FixHeadDescription($htmlData)
...
Func GetLocalImageFiles($html)
	...
	Local $tempImageList = StringRegExp($html, '(?i)[\n\s]src="file:///(.*?)"', 3)
	If @error Then Return SetError(1, 0, 0)
