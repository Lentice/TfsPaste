...
#AutoIt3Wrapper_Res_Fileversion=1.8.0.9
...
Func DoJob()
	...
	If Not StringInStr($title, '- Google Chrome') And Not StringInStr($title, '— Mozilla Firefox') And Not StringInStr($title, '- Microsoft​ Edge') Then
...
Func GetImgTag($html)
	...
	If Not @error Then
		$attrWidth = ' width="' & $width & '"' & ' max-width="' & $width & '"'
	EndIf
...
Func UploadToWeb($filePath)
	...
	If Not _ClipPutFile($filePath) Then Return SetError(-1, 0, 0)
